...
export default function SupportPage() {
    ...
    const fetchTickets = async () => {
        ...
        try {
            const response = await api.get('/support/tickets', { params: { limit: 8 } });
    ...
    const handleStatusChange = async (ticketId: string, status: string) => {
        ...
        try {
            ...
            console.error(error);
            toast({ title: 'Durum guncellenemedi', description: 'Lutfen tekrar deneyin.' });
        } finally {
            setUpdatingId(null);
        }
    };
    ...
    const handleSubmit = async (event: FormEvent) => {
        ...
        try {
            ...
            toast({
                ...
                description: 'Destek ekibi en kisa surede sizinle iletisime gececek.'
            });
            fetchTickets();
        } catch (error) {
            console.error(error);
        ...
        <div className="flex flex-col gap-6 pb-10">
            <section className="rounded-[28px] bg-gradient-to-br from-slate-900 via-slate-950 to-slate-900 p-6 sm:p-8 text-white shadow-2xl shadow-slate-900/40">
                <div className="flex flex-col gap-2">
                    <p className="text-xs font-semibold uppercase tracking-[0.4em] text-emerald-200">Destek merkezi</p>
                    <h2 className="text-3xl font-semibold leading-tight">Kurumsal destek ve talepler</h2>
                    <p className="max-w-3xl text-sm text-white/70">
                        Operasyon, faturalama ya da teknik konular icin tek bir kanaldan destek alin.
                        Talepleriniz onceliklendirilerek takip edilir.
                    </p>
                </div>
            </section>
            ...
            <section className="grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
                <div className="app-card rounded-[24px] border border-slate-100/70 p-5 sm:p-6">
                    <div className="flex items-center justify-between">
                        ...
                        <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-500">7/24</span>
                    </div>
                    <form onSubmit={handleSubmit} className="mt-4 space-y-4">
                        <Input
                            id="subject"
                            label="Konu"
                            placeholder="Ornek: Tahsilat raporu"
                            value={subject}
                ...
                <div className="app-card rounded-[24px] border border-slate-100/70 p-5 sm:p-6 space-y-4">
                    <div>
                        <p className="text-xs font-semibold uppercase tracking-[0.3em] text-slate-400">Gelen talepler</p>
                        <h3 className="text-lg font-semibold text-slate-900">Son destek kayitlari</h3>
                        <p className="text-sm text-slate-500">Sistemden acilan destek taleplerini hizlica inceleyin.</p>
                    </div>
                        ...
                        {loadingTickets ? (
                            ...
                            tickets.map((ticket) => (
                                <div key={ticket.id} className="rounded-2xl border border-slate-100 bg-white/80 px-4 py-3">
                                    ...
                                    <p className="mt-1 text-xs text-slate-500">{ticket.tenant?.name || 'Kurumsal musteri'}</p>
                                    <div className="mt-3 flex items-center justify-between gap-2">
                                        <select
                                            value={ticket.status}
                                            onChange={(event) => handleStatusChange(ticket.id, event.target.value)}
                                            disabled={updatingId === ticket.id}
                                            className="h-9 rounded-md border border-slate-200 bg-slate-50 px-2 text-xs focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/20"
                                        >
                                        ...
                                        {updatingId === ticket.id && (
                                            <span className="text-[11px] text-slate-400">Guncelleniyor...</span>
                                        )}
                                    </div>
                                </div>
                            ))
                    ...
                    <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50 px-4 py-3 text-xs text-slate-500">
                        Kritik talepler icin destek@agito.com adresinden ulasabilirsiniz.
                    </div>
                </div>
            </section>
        </div>
