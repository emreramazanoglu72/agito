        ...
        <div className="flex flex-col gap-6 pb-10">
            <section className="rounded-[28px] bg-gradient-to-br from-slate-900 via-slate-950 to-slate-900 p-6 sm:p-8 text-white shadow-2xl shadow-slate-900/40">
                <div className="flex flex-col gap-2">
                    <p className="text-xs font-semibold uppercase tracking-[0.4em] text-emerald-200">Kurumsal sozlesmeler</p>
                    <h2 className="text-3xl font-semibold leading-tight">Paket & basvuru sozlesmeleri</h2>
                    <p className="max-w-3xl text-sm text-white/70">
                        Onaylanan basvurular uzerinden olusan kurumsal sozlesmeleri takip edin.
                    </p>
                </div>
            </section>
            ...
            <section className="app-card rounded-[24px] border border-slate-100/70 p-5 sm:p-6">
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <h3 className="text-lg font-semibold text-slate-900">Guncel sozlesme listesi</h3>
                    </div>
                ...
                <div className="mt-4 space-y-3">
                    {loading ? (
                        ...
                        items.map((item) => (
                            <button
                                key={item.id}
                                onClick={() => router.push(`/dashboard/corporate-policies/${item.id}`)}
                            ...
                            >
                                <div>
                                    ...
                                    {item.policyType && (
                                        <p className="text-[11px] text-slate-400">Poliçe tipi: {item.policyType}</p>
                                    )}
                                </div>
